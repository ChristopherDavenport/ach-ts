...
export class ADVEntryDetail {
  ...
  addenda99: import('./addenda/addenda99.js').Addenda99 | null = null;
  ...
  parse(record: string): void {
    ...
    this.rdfiIdentification = converters.parseStringField(runes.slice(3, 11).join(''));
    // 12-12 CheckDigit
    this.checkDigit = converters.parseStringField(runes.slice(11, 12).join(''));
    // 13-27 DFIAccountNumber
    this.dfiAccountNumber = runes.slice(12, 27).join('');
    // 28-39 Amount
    this.amount = converters.parseNumField(runes.slice(27, 39).join(''));
    // 40-48 AdviceRoutingNumber
    this.adviceRoutingNumber = converters.parseStringField(runes.slice(39, 48).join(''));
    // 49-53 FileIdentification
    this.fileIdentification = converters.parseStringField(runes.slice(48, 53).join(''));
    // 54-54 ACHOperatorData
    this.achOperatorData = converters.parseStringField(runes.slice(53, 54).join(''));
    ...
    this.achOperatorRoutingNumber = converters.parseStringField(runes.slice(79, 87).join(''));
    // 88-90 JulianDay
    this.julianDay = converters.parseNumField(runes.slice(87, 90).join(''));
    // 91-94 SequenceNumber
    this.sequenceNumber = converters.parseNumField(runes.slice(90, 94).join(''));
  }
  ...
  dfiAccountNumberField(): string { return converters.alphaField(this.dfiAccountNumber, 15); }
  amountField(): string { return converters.numericField(this.amount, 12); }
  adviceRoutingNumberField(): string { return converters.stringField(this.adviceRoutingNumber, 9); }
  fileIdentificationField(): string { return converters.alphaField(this.fileIdentification, 5); }
  achOperatorDataField(): string { return converters.alphaField(this.achOperatorData, 1); }
  individualNameField(): string { return converters.alphaField(this.individualName, 22); }
  discretionaryDataField(): string { return converters.alphaField(this.discretionaryData, 2); }
  achOperatorRoutingNumberField(): string { return converters.alphaField(this.achOperatorRoutingNumber, 8); }
  julianDateDayField(): string { return converters.numericField(this.julianDay, 3); }
