...
import { ARC, CreditsOnly } from '../constants.js';
...
export class BatchARC extends Batch {
  validate(): Error | null {
    ...
    if (err) return err;
    if (this.header.standardEntryClassCode !== ARC) {
      return this.batchError('StandardEntryClassCode', ErrBatchSECType, ARC);
    }
  ...
  validateAll(): Error[] {
    ...
    if (this.header.standardEntryClassCode !== ARC) {
      errors.push(this.batchError('StandardEntryClassCode', ErrBatchSECType, ARC));
    }
    if (this.header.serviceClassCode === CreditsOnly) {
