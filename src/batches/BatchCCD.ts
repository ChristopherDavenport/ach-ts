...
import { CCD } from '../constants.js';
import { ErrBatchSECType, ErrBatchAddendaCount } from '../errors/index.js';

export class BatchCCD extends Batch {
  validate(): Error | null {
    if (this.validateOpts?.skipAll || this.validateOpts?.bypassBatchValidation) return null;
    const err = this.verify();
    if (err) return err;
    if (this.header.standardEntryClassCode !== CCD) {
      return this.batchError('StandardEntryClassCode', ErrBatchSECType, CCD);
    }
  ...
  validateAll(): Error[] {
    ...
    if (this.header.standardEntryClassCode !== CCD) {
      errors.push(this.batchError('StandardEntryClassCode', ErrBatchSECType, CCD));
    }
